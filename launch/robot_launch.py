...
def generate_launch_description():
    ...
    assignment_dir = get_package_share_directory('keepout_zones')
    ...
    robot_description = pathlib.Path(os.path.join(assignment_dir, 'resource', 'tiago_webots.urdf')).read_text()
    ros2_control_params = os.path.join(assignment_dir, 'resource', 'ros2_control.yml')
    nav2_map = os.path.join(assignment_dir, 'maps', 'map.yaml')
    use_sim_time = LaunchConfiguration('use_sim_time', default=True)
    rviz_config = os.path.join(assignment_dir, 'resource', 'default.rviz')

    webots = WebotsLauncher(
        world=PathJoinSubstitution([assignment_dir, 'worlds', world]),
        mode=mode
    )
    ...
    optional_nodes.append(IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(
            assignment_dir, 'launch', 'bringup_launch.py')),
        launch_arguments=[
            ('map', nav2_map),
            ('use_sim_time', use_sim_time),
        ],
    ...
    return LaunchDescription([
        ...
        Node(namespace= "initial_pose" , package='keepout_zones' , executable ='initial_pose' , output='screen'),
    ] + optional_nodes)
